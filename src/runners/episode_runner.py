...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            if hasattr(self.env, 'world') and self.env.world.scenario_name in ['simple_tag', 'simple_adversary', 'simple_world']:
            ...
            if hasattr(self.env, 'world'):
                ...
                elif self.env.world.scenario_name in ['simple_adversary']:
                    opponent_obs = obs[:self.env.num_opponent_agents]
                opponent_obs = th.FloatTensor(np.expand_dims(opponent_obs, 1))#.cuda()
            
                opponent_actions = th.cat([self.opponents[i].step(opponent_obs[i], explore=False) for i in range(self.env.num_opponent_agents)
                ])
                opponent_actions = th.argmax(opponent_actions, 1) # convert one-hot to int
                opponent_actions = th.tensor(np.expand_dims(opponent_actions.cpu(), 0))#.cuda()
