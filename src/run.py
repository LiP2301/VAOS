...
class MLPNetwork(nn.Module):
    ...
    def __init__(self, input_dim, out_dim, hidden_dim=64, nonlin=F.relu, constrain_out=False, norm_in=True,
...
def onehot_from_logits(logits, eps=0.0, dim=1):
    ...
    rand_acs = Variable(th.eye(logits.shape[1])[[np.random.choice(range(logits.shape[1]), size=logits.shape[0])]],
                        requires_grad=False)
...
def load_pretrained_opponents(opponents, scenario_name, filename, num_cooperating_agents, num_opponent_agents,
                              device='gpu'):
...
def run_sequential(args, logger):
    ...
    if args.vaos: scheme['future_discounted_return'] = {"vshape": (1,)}
    ...
    if args.env == 'mpe_env':
        ...
        pretrained_opponent_model_dir = '/VAOS/src/pretrained_models/{}_{}_episode_{}.pt'.format(
            args.scenario_name,
            args.opponent_algo,
            args.load_opponent_model_episode
        )
        load_pretrained_opponents(opponents, args.scenario_name, pretrained_opponent_model_dir, num_cooperating_agents,
                                  num_opponent_agents)
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_log_T) >= args.log_interval:
            ...
            # print(logger.sacred_info.get("return_mean"))
            # print("return_mean", np.mean(logger.sacred_info.get("return_mean")))
